...
int Equipement::ExecuterMessage(MessageTexte* message){
	...
	*/
	...
	if (0 == (ofs = message->LireStrCrc(ofs, ';', &equipId)) || equipId != this->id){
		// recherche un équipement dans les coms
		ComObjet* com = this->GetEquipement(equipId);
		if (com != 0){
			Serial.println("Send message by com...");
			if (com->serialReady == false)
				Serial.println("Com not ready !");
			else if (com->serial != 0){
				Serial.println("Send message...");
				com->serial->write((const uint8_t*)message->GetBuffer(), strlen(message->GetBuffer()));
				return 1;
			}
		}
...
// Obtient un objet
ComObjet* Equipement::GetEquipement(unsigned int equipId){
	Objet* list[EQUIPEMENT_MAX_OBJET];
	ComObjet* com = 0;
	int maxObjet = Objets(list);
	unsigned int ComObjetType = crc32("ComObjet");
	for (int x = 0; x<maxObjet; x++){
		if (ComObjetType == list[x]->GetType()){
			com = (ComObjet*)list[x];
			if (com->equipId == equipId)
				return com;
		}
//		ComObjet* com = dynamic_cast<ComObjet*> (list[x]);
//		if (com != 0 && com->equipId == equipId)
//			return list[x];
	}
	return 0;
}
